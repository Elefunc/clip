...
- `board/`
  - `board.c`: Win32 GUI clipboard viewer entry point, clipboard format detection, text display, bitmap image rendering, DPI-aware image zoom/window sizing, font selection, tooltip text, and single-instance handling. Keep related helpers local until the file becomes too large to navigate safely.
  ...
  - `Makefile`: Dedicated build script for `board32.exe` and `board64.exe`, storing intermediate objects under `board/obj/`, leaving the executables in place, rebuilding resources when icon/manifest/resource inputs change, and signing rebuilt executables.
...
- `cd board && make`: Builds `board64.exe` and `board32.exe` with the same toolchain defaults, links Direct2D/WIC/common-controls dependencies, and automatically code-signs rebuilt executables with `cs`. Pair with `cd board && make clean` when tidying the workspace.
...
- For Board, run `board/board64.exe` on Windows, then copy text and confirm the title uses `# chars (<encoding>)`, the text area uses the monospace fallback order, and the font tooltip lists the three families. Copy a bitmap image and confirm the title uses `W×H - #%`, double-click resets to 100%, `0`/`1`-`9` and `Alt+1`-`Alt+9` zoom shortcuts work, and the window remains DPI/aspect-ratio aware. Copy unsupported content and confirm the unsupported-format message appears without crashing.
