...
- `cs trim/trim64.exe trim/trim32.exe paste/paste64.exe paste/paste32.exe`: Sign one or more rebuilt executables with the local Azure Trusted Signing wrapper before publishing release artifacts.
...
2. **Make a clean build**
   ...
   - Run `cd paste && make clean && make` to rebuild `paste32.exe` and `paste64.exe`; confirm both executables are produced without warnings.
3. **Sign the binaries**
   - Run `cs trim/trim64.exe trim/trim32.exe paste/paste64.exe paste/paste32.exe` from the repository root.
   - `cs` accepts one or more files and signs them with Azure Trusted Signing.
...
- Use the local `cs` wrapper for code signing instead of calling `sign.exe` directly; `cs <file> [file ...]` signs one or more files with Azure Trusted Signing.
